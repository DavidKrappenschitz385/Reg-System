<?php
  ...
  if (isset($_POST['updateplayer'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
      ...
      $stmt = mysqli_prepare($db_con, "SELECT `player_id` FROM `players` WHERE `id` = ?");
      mysqli_stmt_bind_param($stmt, "i", $id);
      mysqli_stmt_execute($stmt);
      $player_id_query = mysqli_stmt_get_result($stmt);
    ...
    $stmt = mysqli_prepare($db_con, "UPDATE `players` SET `full_name` = ?, `age` = ?, `gender` = ?, `sport_id` = ?, `team_id` = ?, `contact_number` = ?, `address` = ?, `photo` = ? WHERE `id` = ?");
    mysqli_stmt_bind_param($stmt, "sisiisssi", $full_name, $age, $gender, $sport_id, $team_id, $contact_number, $address, $photo_new, $id);
...
<?php
  if (isset($id)) {
    $stmt = mysqli_prepare($db_con, "SELECT * FROM `players` WHERE `id` = ?");
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
